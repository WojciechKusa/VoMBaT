...
TPR_B[:_model_b_change_position] = [x/910 for x in list(range(_model_b_change_position))]
...
plt.plot(new_FPR, TPR_A, color='cornflowerblue', lw=1.5, label='Model A ROC curve (area = %0.2f)' % roc_auc_a)
plt.plot(new_FPR, TPR_B, color='crimson', lw=1.5, label='Model B ROC curve (area = %0.2f)' % roc_auc_b)
...
fig, ax = plt.subplots(figsize=(7, 4.5))
plt.plot(new_TNR, TPR_A, color='cornflowerblue', lw=1.5, label='Model A ROC curve (area = %0.2f)' % roc_auc_a)
plt.plot(new_TNR, TPR_B, color='crimson', lw=1.5, label='Model B ROC curve (area = %0.2f)' % roc_auc_b)
plt.plot([0, 1], [0, 1], color='coral', lw=1.5, linestyle='--')
plt.grid(axis='y', alpha=0.5)
plt.ylim([0.0, 1.001])
plt.xlim([0.0, 1.0])
